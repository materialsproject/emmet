...
module load esslurm;
out=`squeue -u mwu1011 -h --format "%.7i %.2t"`;
job_id=`echo $out | cut -f1 -d " "`;
job_status=`echo $out | cut -f2 -d " "`;

if [ -z "$job_id"  ]
then
        echo "no job is running, clear to run upload latest pipeline";
	emmet --sbatch --yes --run --issue 87 tasks -d /global/cscratch1/sd/mwu1011/projects upload-latest -n 4500; 
else 
	echo "the following job id and status is running, cannot run the next upload"
