...
def test_create(tmp_dir):
    ...
    assert "Must provide at least one file or directory path to include in submission" in str(result.exception)
...
def test_add_to(sub_file):
    ...
    assert result.exit_code != 0
    assert isinstance(result.exception, EmmetCliError)
    assert "Must provide at least one file or directory path to add to the submission" in str(result.exception)

    sub = Submission.load(Path(sub_file))
    it = iter(sub.calculations)
    while (path_to_add_from := next(it), None) and "neb_calc/01" not in str(path_to_add_from):
        pass
...
def test_remove_from(sub_file):
    ...
    assert result.exit_code != 0
    assert isinstance(result.exception, EmmetCliError)
    assert "Must provide at least one file or directory path to remove from the submission" in str(result.exception)

    sub = Submission.load(Path(sub_file))
    it = iter(sub.calculations)
    while (path_to_remove := next(it), None) and "neb_calc/01" not in str(path_to_remove):
        pass
