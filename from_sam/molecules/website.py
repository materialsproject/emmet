...
class WebsiteMoleculesBuilder(Builder):
    def __init__(self, molecules, redox, website, query=None, **kwargs):
        """
    ...
    def get_items(self):
        ...
        redox_task_ids = self.redox.distinct(
            self.redox.key,
            {"$or": [{"redox.IE": {"$exists": 1}}, {"redox.EA": {"$exists": 1}}]},
        )
    ...
    def process_item(self, item):
        ...
        if "EA" in item["redox"]:
            if "solvated" in item["redox"]["EA"]:
                ...
                doc["electrode_potentials"]["reduction"]["lithium"] = item["redox"][
                    "reduction"
                ]["solvated"]["Li"]
                doc["electrode_potentials"]["reduction"]["hydrogen"] = item["redox"][
                    "reduction"
                ]["solvated"]["H"]
                doc["electrode_potentials"]["reduction"]["magnesium"] = item["redox"][
                    "reduction"
                ]["solvated"]["Mg"]
        ...
        if "IE" in item["redox"]:
            if "solvated" in item["redox"]["IE"]:
                ...
                doc["electrode_potentials"]["oxidation"]["lithium"] = item["redox"][
                    "oxidation"
                ]["solvated"]["Li"]
                doc["electrode_potentials"]["oxidation"]["hydrogen"] = item["redox"][
                    "oxidation"
                ]["solvated"]["H"]
                doc["electrode_potentials"]["oxidation"]["magnesium"] = item["redox"][
                    "oxidation"
                ]["solvated"]["Mg"]
        ...
        if "EA" in item["redox"] and "IE" in item["redox"]:
            if "solvated" in item["redox"]["EA"]:
                doc["electrochemical_window_width"] = (
                    item["redox"]["EA"]["solvated"] - item["redox"]["IE"]["solvated"]
                )
            elif "vacuum" in item["redox"]["EA"]:
                doc["electrochemical_window_width"] = (
                    item["redox"]["EA"]["vacuum"] - item["redox"]["IE"]["vacuum"]
                )
...
def xyz2svg(xyz):
    ...
    p = subprocess.Popen(
        babel_cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def modify_svg(svg):
    ...
    for line in tokens:
        if "line" in line:
            line = re.sub('stroke-width="\d+.?\d*"', 'stroke-width="3.0"', line)
        if "rect" in line:
