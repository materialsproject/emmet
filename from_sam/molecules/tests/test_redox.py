...
test_tasks = os.path.join(
    module_dir, "..", "..", "..", "test_files", "sample_qchem_tasks.json"
)
...
class TestRedox(unittest.TestCase):
    ...
    def test_get_and_process(self):
        ...
        for group in grouped_mols:
            ...
            for mol in mols:
                if mol["formula_alphabetical"] == "C2 H4" or (
                    mol["formula_alphabetical"] == "C6 H8 O2" and mol["charge"] == 1
                ):
                    self.assertEqual("redox" not in mol, True)
                if mol["formula_alphabetical"] == "B1 H4":
                    if mol["charge"] == 0:
                        self.assertEqual(
                            mol["redox"]["reduction"]["solvated"]["Li"],
                            4.512541192925232,
                        )
                    elif mol["charge"] == -1:
                        self.assertEqual(
                            mol["redox"]["oxidation"]["solvated"]["Li"],
                            4.512541192925232,
                        )
