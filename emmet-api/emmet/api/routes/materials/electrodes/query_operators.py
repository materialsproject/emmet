...
from emmet.api.routes.materials.electrodes.utils import electrodes_formula_to_criteria, electrodes_chemsys_to_criteria
...
Wildcards for unknown elements only supported for single chemsys queries",
    ...
    def ensure_indexes(self):  # pragma: no cover
        keys = ["entries_composition_summary.all_chemsys", "entries_composition_summary.all_elements", "nelements"]
    ...
    def query(
        ...
        if elements:
            try:
                ...
                raise HTTPException(status_code=400, detail="Problem processing one or more provided elements.")
