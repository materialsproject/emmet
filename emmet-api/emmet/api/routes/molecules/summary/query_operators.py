...
class MPculeIDsSearchQuery(QueryOperator):
    ...
    def query(
        self,
        molecule_ids: Optional[str] = Query(None, description="Comma-separated list of molecule_ids to query on"),
    ) -> STORE_PARAMS:
    ...
    def post_process(self, docs, query):
        ...
        if not query.get("sort", None):
            mpcule_ids = query.get("criteria", {}).get("molecule_id", {}).get("$in", None)

            if mpcule_ids is not None and "molecule_id" in query.get("properties", []):
                mpculeid_mapping = {mpculeid: ind for ind, mpculeid in enumerate(mpcule_ids)}
