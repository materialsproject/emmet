...
def test_formula_to_criteria():
    # Regular formula
    assert formula_to_criteria("Cr2O3") == {"composition_reduced.Cr": 2.0, "composition_reduced.O": 3.0, "nelements": 2}
    assert formula_to_criteria("Cr2O3, O2Si") == {"formula_pretty": {"$in": ["Cr2O3", "SiO2"]}}

    # Add wildcard
    assert formula_to_criteria("Cr2*3") == {"composition_reduced.Cr": 2.0, "formula_anonymous": "A2B3"}
    # Anonymous element
