...
def test_formula_query():
    ...
    assert op.query("Si2O4") == {
        "criteria": {"composition_reduced.O": 2.0, "composition_reduced.Si": 1.0, "nelements": 2}
    }
...
def test_symmetry_query():
    op = SymmetryQuery()
    assert op.query(crystal_system=CrystalSystem.cubic, spacegroup_number=221, spacegroup_symbol="Pm3m") == {
        "criteria": {"symmetry.crystal_system": "Cubic", "symmetry.number": 221, "symmetry.symbol": "Pm3m"}
    }

    with ScratchDir("."):
        dumpfn(op, "temp.json")
        new_op = loadfn("temp.json")
        assert new_op.query(crystal_system=CrystalSystem.cubic, spacegroup_number=221, spacegroup_symbol="Pm3m") == {
            "criteria": {"symmetry.crystal_system": "Cubic", "symmetry.number": 221, "symmetry.symbol": "Pm3m"}
        }
...
def test_find_structure_query():
    ...
    assert op.post_process(docs, query) == [
        {"material_id": "mp-149", "normalized_rms_displacement": 0, "max_distance_paired_sites": 0}
    ]
...
def test_formula_auto_complete_query():
    ...
    pipeline = [
        {"$search": {"index": "formula_autocomplete", "text": {"path": "formula_pretty", "query": ["SiO", "OSi"]}}},
    ...
    pipeline = [
        {"$search": {"index": "formula_autocomplete", "text": {"path": "formula_pretty", "query": ["Si"]}}},
        {"$project": {"_id": 0, "formula_pretty": 1, "elements": 1, "length": {"$strLenCP": "$formula_pretty"}}},
