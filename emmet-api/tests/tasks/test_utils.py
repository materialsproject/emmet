...
def test_calcs_reversed_to_trajectory():
    with open(os.path.join(MAPISettings(DB_VERSION="").TEST_FILES, "calcs_reversed_mp_1031016.json")) as file:
...
def test_task_to_entry():
    with open(os.path.join(MAPISettings(DB_VERSION="").TEST_FILES, "test_task.json")) as file:
        doc = load(file)
