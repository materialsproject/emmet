from emmet.api.routes.materials.bonds.query_operators import BondLengthQuery, CoordinationEnvsQuery
...
def test_bond_length_query_operator():
    ...
    fields = ["bond_length_stats.min", "bond_length_stats.max", "bond_length_stats.mean"]
...
def test_coordination_envs_query():
    ...
    assert op.query(coordination_envs="Mo-S(6),S-Mo(3)", coordination_envs_anonymous="A-B(6),A-B(3)") == {
        "criteria": {
    ...
    with ScratchDir("."):
        dumpfn(op, "temp.json")
        new_op = loadfn("temp.json")
        assert new_op.query(coordination_envs="Mo-S(6),S-Mo(3)", coordination_envs_anonymous="A-B(6),A-B(3)") == {
            "criteria": {
                "coordination_envs": {"$all": ["Mo-S(6)", "S-Mo(3)"]},
                "coordination_envs_anonymous": {"$all": ["A-B(6)", "A-B(3)"]},
            }
