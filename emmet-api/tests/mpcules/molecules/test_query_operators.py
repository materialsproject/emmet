...
def test_multi_mpculeid_query():
    ...
    assert op.query(
        mpcule_ids="21d752f3018fd3c4eba7a9ce7a37b8c8-C1F1Mg1N1O1S2-1-2, 542d9adc3163002fe8dfe6d226875dde-C3H5Li2O3-0-2"
    ) == {
        "criteria": {
            "molecule_id": {
                "$in": [
...
def test_find_molecule_query():
    ...
    mol = Molecule.from_file(os.path.join(MAPISettings(DB_VERSION="").TEST_FILES, "test_molecule.xyz"))
    query = {
        "criteria": {
    ...
    assert op.query(molecule=mol.as_dict(), tolerance=0.01, charge=0, spin_multiplicity=1, _limit=1) == query
...
def test_find_molecule_connectivity_query():
    ...
    assert op.query(molecule=mol.as_dict(), charge=0, spin_multiplicity=1, _limit=1) == query
...
def test_hash_query():
    ...
    assert op.query(species_hash="ea83c62377feef8c8c3190562e13ffd6", coord_hash="5a0282381090c5c9646d03891133d8c9") == {
        "criteria": {
    ...
    with ScratchDir("."):
        ...
        new_op = loadfn("temp.json")
        assert new_op.query(
            species_hash="ea83c62377feef8c8c3190562e13ffd6", coord_hash="5a0282381090c5c9646d03891133d8c9"
        ) == {
            "criteria": {
                "species_hash": "ea83c62377feef8c8c3190562e13ffd6",
                "coord_hash": "5a0282381090c5c9646d03891133d8c9",
