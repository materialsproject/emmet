...
class PhononBuilder(Builder):
    def __init__(self, materials, phonon, phonon_bs, phonon_dos, ddb_files, msqd_dos,
                 query=None, manager=None, **kwargs):
        """
        ...
        Args:
            ...
            ddb_files (Store): target Store of the DDB files. The document may
                exceed the 16MB limit of the mongodb collections.
            msqd_dos (Store): target Store of the data related to the generalized phonon DOS
                with the mean square displacement tensor. The document may exceed the 16MB
                limit of the mongodb collections.
        ...
        super().__init__(sources=[materials],
                         targets=[phonon, phonon_bs, phonon_dos, ddb_files, msqd_dos],
                         **kwargs)
    ...
    def get_phonon_properties(self, item):
        ...
        with tempfile.TemporaryDirectory() as workdir:
            ...
            with task.open_phbst() as phbst_file, AnaddbNcFile(os.path.join(workdir, "anaddb.nc")) as ananc_file:
                ...
                e_total = ananc_file.eps0.tolist()
            ...
            ir_spectra = IRDielectricTensor(dielectric.oscillator_strength, dielectric.phfreqs,
    ...
    def update_targets(self, items):
        ...
        items_msqd_dos = [{self.msqd_dos.key: i[self.phonon.key],
                          "msqd_dos": i['phonon'].pop('msqd_dos')} for i in items]
        ...
        if len(items) > 0:
            ...
            self.msqd_dos.update(docs=items_msqd_dos)
        else:
