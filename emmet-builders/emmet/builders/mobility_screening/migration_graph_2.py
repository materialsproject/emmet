...
class MigrationGraphBuilder(MapBuilder):
    ...
    def unary_function(self, item: dict) -> dict:
        ...
        #limit max_distance_max to primitve cell
        try:
            lattice = Structure.from_dict(item["host_structure"]).get_primitive_structure().lattice
            max_distance_max = min(max(lattice.a, lattice.b, lattice.c)*1.1, 7)
        except:
            print("something went wrong with max_distance_max")
            max_distance_max = 7
    ...
    #hop cut-off at max hop_cost / min hop_cost >= 2  OR spike in hops_count
    def get_migration_graph(
        ...
        try:
            ...
            if len(mg_structures) > 0:
                ...
                spike = False
                hops_count = 0
                while cost_limit and max_distance < max_distance_max and not spike:
                    ...
                    if mg_trial.m_graph.graph.number_of_edges() > 0:
                        ...
                        if len(paths) == 0:
                            ...
                            min_hop_cost = min(
                                mg_trial.unique_hops.values(), key=lambda x: x["cost"]
                            )["cost"]
                            hops_count = len(mg_trial.unique_hops)
                            mg = mg_trial
                            max_distance = max_distance * 1.1
                        elif (
                            hops_count < 4
                            or len(mg_trial.unique_hops) < 1.5 * hops_count
                        ):
                            hops_count = len(mg_trial.unique_hops)
                            curr_max_hop_cost = max(
                                mg_trial.unique_hops.values(), key=lambda x: x["cost"]
                            )["cost"]
                            if curr_max_hop_cost >= (min_hop_cost * 2):
                                cost_limit = False
                            else:
                                max_distance = max_distance * 1.1
                            mg = mg_trial
