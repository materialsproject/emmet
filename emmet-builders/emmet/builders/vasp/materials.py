...
class DMaterialsBuilder(MaterialsBuilder):

    def get_items(self) -> Iterator[List[Dict]]:
        all_tasks, to_process_tasks = self._get_tasks_to_process()

        print(f"Found {len(to_process_tasks)} unprocessed tasks")

        to_process_forms = to_process_tasks["formula_pretty"].unique()

        print(f"Found {len(to_process_forms)} unprocessed formulas")
        ...
        return to_process_tasks

    def manual_chunks(self, grouped):
        ret = []
        temp = []
    ...
    def _partition_work(self, df):
        grouped = df.groupby(["formula_pretty"])

        chunked = self.manual_chunks(grouped)

        return chunked

    def _get_tasks_to_process(self):
        task_types = [t.value for t in self.settings.VASP_ALLOWED_VASP_TYPES]
        self.logger.info("Materials builder started")
        self.logger.info(f"Allowed task types: {task_types}")
        ...
        self.logger.info("Finding tasks to process")
        # Get all processed tasks:
        # filter to only successful
        all_tasks = self.tasks.query(
            criteria={"query": "state == 'successful'"},
            properties=self.tasks.searchable_fields,
            criteria_fields=["state"],
        )
        print(f"All successful tasks:\n{all_tasks}")

        # filter to the ones that include build tags and don't include exclude tags
        all_tasks = all_tasks[
            all_tasks["tags"].apply(
                lambda x: any(tag in x for tag in self.settings.BUILD_TAGS)
            )
        ]
        all_tasks = all_tasks[
            ~all_tasks["tags"].apply(
                lambda x: any(tag in self.settings.EXCLUDED_TAGS for tag in x)
            )
        ]

        print(f"All tasks from tasks query:\n{all_tasks}")
        processed_tasks = set()

        try:
            for task_list in self.materials.query(properties=["task_ids"])["task_ids"]:
                processed_tasks.update(task_list)
        except KeyError:
            self.logger.warning("Missing keys in manifest, assuming bucket is empty")

        print(f"Found {len(processed_tasks)} already processed tasks")
        return all_tasks, all_tasks[~all_tasks["task_id"].isin(processed_tasks)]
    ...
    def get_processed_docs(self, mats):
        ...
        for df in mats:
            temp_df = []
            tasks = df.groupby(self.tasks.object_grouping)

            for group, group_tasks in tasks:
                path = self.tasks._get_full_key_path(group_tasks)
                df = self.tasks._read_doc_from_s3(path)
                df = df[df["task_id"].isin(group_tasks["task_id"])]
                temp_df.append(df)

                # for t in tasks:
                #     t["is_valid"] = t[self.tasks.key] not in invalid_ids
            all_docs.append(pd.concat(temp_df))
    ...
    def update_targets(self, items: List[List[Dict]]):
        ...
        if len(docs) > 0:
            self.logger.info(f"Updating {len(docs)} materials")
            self.materials.update(docs=pd.DataFrame.from_records(docs))
        else:
            self.logger.info("No items to update")
