...
class DefectBuilder(Builder):
    ...
    def get_items(self):
        ...
        q.update(
            {
                "transformations.history.@module": {
        ...
        if self.max_items_size and len(defect_tasks) > self.max_items_size:
            defect_tasks = [
                dtask
                for dind, dtask in enumerate(defect_tasks)
                if dind < self.max_items_size
            ]
        task_ids = [dtask["task_id"] for dtask in defect_tasks]
        self.logger.info(
            "Found {} new defect tasks to consider:\n{}".format(
                len(defect_tasks), task_ids
            )
        )
        ...
        dielq = {
            "state": "successful",
            "input.incar.LEPSILON": True,
            "input.incar.LPEAD": True,
        }
        HSE_BSq = {
            "state": "successful",
            "calcs_reversed.0.input.incar.LHFCALC": True,
            "transformations.history.0.@module": {
                "$nin": [
        ...
        log_additional_tasks = (
            dict()
        )  # to minimize number of bulk + diel + hse queries, log by chemsys

        needed_defect_properties = [
            "task_id",
            "transformations",
            "input",
            "task_label",
        ...
        for d_task_id in task_ids:
            d_task = list(
                self.tasks.query(
                    criteria={"task_id": d_task_id}, properties=needed_defect_properties
                )
            )[0]
            chemsys = "-".join(
                sorted(
                    (
                        Structure.from_dict(
                            d_task["transformations"]["history"][0]["defect"][
                                "structure"
                            ]
                        ).symbol_set
                    )
            ...
            if chemsys not in log_additional_tasks.keys():
                ...
                bulk_tasks = list(
                    self.tasks.query(criteria=q, properties=needed_bulk_properties)
                )
                ...
                self.logger.debug(
                    "\t{} has {} bulk loaded {} diel and {} hse"
                    "".format(
                        chemsys, len(bulk_tasks), len(diel_tasks), len(hybrid_tasks)
                    )
                )

                log_additional_tasks.update(
                    {
                        chemsys: {
                            "bulksc": bulk_tasks[:],
                            "diel": diel_tasks[:],
    ...
    def process_item(self, item):
        ...
        if defect_task is None:
            self.logger.error(
                "Could not determine defect bulk properties for {}".format(
                    d_task["task_id"]
                )
            )
            return
            ...
            self.logger.error(
                "bulk_task is not in item! Cannot parse task id = {}.".format(
                    d_task["task_id"]
                )
            )
            return
            ...
            self.logger.error(
                "diel_task_meta is not in item! Cannot parse task id = {}.".format(
                    d_task["task_id"]
                )
            )
            return
        ...
        parameters = {
            "epsilon_ionic": eps_ionic,
            "epsilon_static": eps_static,
            "dielectric": eps_total,
            "task_level_metadata": {
                "diel_taskdb_task_id": defect_task["diel_task_meta"][
                    "diel_taskdb_task_id"
                ]
            },
        }
        ...
        defect, parameters = self.load_defect_and_structure_data(
            defect_task, parameters
        )

        parameters = self.load_defect_chg_correction_metadata(
            defect, defect_task, parameters
        )

        if "vr_eigenvalue_dict" in defect_task["calcs_reversed"][0]["output"].keys():
            eigenvalues = defect_task["calcs_reversed"][0]["output"][
                "vr_eigenvalue_dict"
            ]["eigenvalues"]
            kpoint_weights = defect_task["calcs_reversed"][0]["output"][
                "vr_eigenvalue_dict"
            ]["kpoint_weights"]
            parameters.update(
            ...
            self.logger.error(
                "DEFECTTYPEcalc: {} (task-id {}) does not have eigenvalue data for parsing "
        ...
        if "defect" in defect_task["calcs_reversed"][0]["output"].keys():
            parameters.update(
                {
                    "defect_ks_delocal_data": defect_task["calcs_reversed"][0][
                        "output"
                    ]["defect"].copy()
                }
            )
            ...
            self.logger.error(
                "DEFECTTYPEcalc: {} (task-id {}) does not have defect data for parsing "
                "delocalization.".format(
                    defect_task["task_label"], defect_task["task_id"]
                )
            )

        defect_entry = DefectEntry(
            defect,
            parameters["defect_energy"] - parameters["bulk_energy"],
            corrections={},
            parameters=parameters,
            entry_id=defect_task["task_id"],
        )

        defect_entry = self.compatibility.process_entry(defect_entry)
        defect_entry.parameters = jsanitize(
            defect_entry.parameters, strict=True, allow_bson=True
        )
        defect_entry_as_dict = defect_entry.as_dict()
        defect_entry_as_dict["task_id"] = defect_entry_as_dict[
            "entry_id"
        ]  # this seemed neccessary for legacy db
    ...
    def find_and_load_bulk_tasks(self, defect_task, additional_tasks):
        ...
        bulk_matched = []
        dstruct_withoutdefect = Structure.from_dict(
            out_defect_task["transformations"]["history"][0]["defect"]["structure"]
        )
        scaling_matrix = out_defect_task["transformations"]["history"][0][
            "scaling_matrix"
        ]
        dstruct_withoutdefect.make_supercell(scaling_matrix)
        dincar = out_defect_task["calcs_reversed"][0]["input"][
            "incar"
        ]  # identify essential INCAR properties which differentiate different calcs
        dincar_reduced = {
            k: dincar.get(k, None)
            if dincar.get(k) not in ["None", "False", False]
            else None
            for k in ["LHFCALC", "HFSCREEN", "IVDW", "LUSE_VDW", "LDAU", "METAGGA"]
        }
        d_potcar_base = {
            "pot_spec": [
                potelt["titel"] for potelt in out_defect_task["input"]["potcar_spec"]
            ],
        ...
        for b_task in bulk_tasks:
            ...
            if bulk_sm.fit(bstruct, dstruct_withoutdefect):
                ...
                bincar_reduced = {
                    k: bincar.get(k, None)
                    if bincar.get(k) not in ["None", "False", False]
                    else None
                ...
                b_potcar = {
                    "pot_spec": set(
                        [potelt["titel"] for potelt in b_task["input"]["potcar_spec"]]
                    ),
                ...
                d_potcar = (
                    d_potcar_base.copy()
                )  # need to reduce in cases of extrinsic species or reordering
                d_potcar["pot_spec"] = set(
                    [d for d in d_potcar["pot_spec"] if d in b_potcar["pot_spec"]]
                )
                d_potcar["pot_labels"] = set(
                ...
                for bsite_coords in bstruct.cart_coords:
                    if not len(
                        dstruct_withoutdefect.get_sites_in_sphere(bsite_coords, 0.1)
                    ):
                        same_cart_positions = False

                if (
                    bincar_reduced == dincar_reduced
                    and b_potcar == d_potcar
                    and same_cart_positions
                ):
                    bulk_matched.append(b_task.copy())
                else:
                    try:
                        self.logger.debug(
                            "Bulk structure match was found for {} with {}, "
                            "but:".format(
                                b_task["task_label"], out_defect_task["task_label"]
                            )
                        )
                    except:
                        self.logger.debug(
                            "BULK STRUCT MATCH was found, but task_label did not exist AND:"
                        )
                    if bincar_reduced != dincar_reduced:
                        out_inc = {
                            k: [v, bincar_reduced[k]]
                            for k, v in dincar_reduced.items()
                            if v != bincar_reduced[k]
                        }
                        self.logger.debug(
                            "\tIncars were different: {} ".format(out_inc)
                        )
                    if b_potcar != d_potcar:
                        out_pot = {
                            k: [v, b_potcar[k]]
                            for k, v in d_potcar.items()
                            if v != b_potcar[k]
                        }
                        self.logger.debug(
                            "\tPotcar specs were different: {} ".format(out_pot)
                        )
        ...
        if len(bulk_matched):
            bulk_matched = sorted(
                bulk_matched, key=lambda x: x["last_updated"], reverse=True
            )
            out_defect_task["bulk_task"] = bulk_matched[0].copy()
            self.logger.debug(
                "Found {} possible bulk supercell structures. Taking most recent entry updated "
                "on: {}".format(len(bulk_matched), bulk_matched[0]["last_updated"])
            )
        else:
            self.logger.error(
                "Bulk task doesnt exist for: {} ({})! Cant create defect "
                "object...\nMetadata: {}\n{}".format(
            ...
            if diel_sm.fit(diel_struct, dstruct_withoutdefect):
                ...
                diel_potcar = {
                    "pot_spec": set(
                        [
                            potelt["titel"]
                            for potelt in diel_task["input"]["potcar_spec"]
                        ]
                    ),
                    "pot_labels": set(
                        diel_task["input"]["pseudo_potential"]["labels"][:]
                    ),
                ...
                d_potcar["pot_spec"] = set(
                    [d for d in d_potcar["pot_spec"] if d in diel_potcar["pot_spec"]]
                )
                d_potcar["pot_labels"] = set(
                    [
                        d
                        for d in d_potcar["pot_labels"]
                        if d in diel_potcar["pot_labels"]
                    ]
                )
                ...
                if diel_potcar == d_potcar:
                    ...
                    try:
                        self.logger.debug(
                            "Dielectric structure match was found for {} with {}, "
                            "but:".format(
                                diel_task["task_label"], out_defect_task["task_label"]
                            )
                        )
                    except:
                        self.logger.debug(
                            "Dielectric STRUCT MATCH was found, but task_label did not exist AND:"
                        )
                    out_pot = {
                        k: [v, diel_potcar[k]]
                        for k, v in d_potcar.items()
        ...
        if len(diel_matched):
            diel_matched = sorted(
                diel_matched, key=lambda x: x["last_updated"], reverse=True
            )
            diel_dict = {
                "diel_taskdb_task_id": diel_matched[0]["task_id"],
            ...
            self.logger.debug(
                "Found {} possible dieletric calcs. Taking most recent entry updated "
                "on: {}".format(len(diel_matched), diel_matched[0]["last_updated"])
            )
        else:
            try:
                self.logger.error(
                    "Dielectric task doesnt exist for: {} ({})! Cant create defect "
                    "object...\nMetadata for defect: {}\n{}".format(
                        out_defect_task["task_label"],
                        out_defect_task["task_id"],
                        d_potcar,
        ...
        if dincar_reduced["LHFCALC"] in [None, False, "False"] and len(
            additional_tasks["hsebs"]
        ):
            hse_bs_matched = []
            for hse_bs_task in additional_tasks["hsebs"]:
                hse_bs_struct = Structure.from_dict(hse_bs_task["input"]["structure"])
                if diel_sm.fit(
                    hse_bs_struct, dstruct_withoutdefect
                ):  # can use same matching scheme as the dielectric structure matcher
                    hse_bs_matched.append(hse_bs_task.copy())
                else:
                    self.logger.debug(
                        "task id {} has a structure which did not match {} for use "
                        "as an HSE BS calculation".format(
                            hse_bs_task["task_id"], out_defect_task["task_label"]
                        )
                    )

            if len(
                hse_bs_matched
            ):  # match the lowest CBM  and highest VBM values, keeping track of their task_ids
                hybrid_cbm_data = min(
                    [
                        [htask["output"]["cbm"], htask["task_id"]]
                        for htask in hse_bs_matched
                    ]
                )
                hybrid_vbm_data = max(
                    [
                        [htask["output"]["vbm"], htask["task_id"]]
                        for htask in hse_bs_matched
                    ]
                )
                hybrid_meta = {
                    "hybrid_cbm": hybrid_cbm_data[0],
                    "hybrid_CBM_task_id": hybrid_cbm_data[1],
                    "hybrid_vbm": hybrid_vbm_data[0],
                    "hybrid_VBM_task_id": hybrid_vbm_data[1],
                }
                out_defect_task["hybrid_bs_meta"] = hybrid_meta.copy()
                self.logger.debug(
                    "Found hybrid band structure properties for {}:\n\t{}".format(
                        out_defect_task["task_label"], hybrid_meta
                    )
                )
            else:
                self.logger.debug(
                    "Could NOT find hybrid band structure properties for {} despite "
                    "there being {} eligible hse calculations".format(
                        out_defect_task["task_label"], len(additional_tasks["hsebs"])
                    )
                )
    ...
    def get_run_metadata(self, item, parameters):
        ...
        potcar_summary = {
            "pot_spec": list(
                [potelt["titel"] for potelt in item["input"]["potcar_spec"]]
            ),
            "pot_labels": list(item["input"]["pseudo_potential"]["labels"][:]),
            "pot_type": item["input"]["pseudo_potential"]["pot_type"],
            "functional": item["input"]["pseudo_potential"]["functional"],
        }  # note bulk has these potcar values also, other wise it would not get to process_items
        ...
        parameters["task_level_metadata"].update(
            {
                "defect_dir_name": dir_name,
        ...
        if (dincar_reduced["HFSCREEN"] in [None, False, "False"]) and (
            "hybrid_bs_meta" in item
        ):
            parameters.update(
                {k: item["hybrid_bs_meta"][k] for k in ["hybrid_cbm", "hybrid_vbm"]}
            )
            parameters.update(
                {"hybrid_gap": parameters["hybrid_cbm"] - parameters["hybrid_vbm"]}
            )
            parameters["task_level_metadata"].update(
                {
                    k: item["hybrid_bs_meta"][k]
                    for k in ["hybrid_CBM_task_id", "hybrid_VBM_task_id"]
                }
            )
        return parameters
    ...
    def get_bulk_chg_correction_metadata(self, bulk_task, parameters):
        if "locpot" in bulk_task["calcs_reversed"][0]["output"].keys():
            ...
            self.logger.error(
                "BULKTYPEcalc: {} (task-id {}) does not "
                "have locpot values for parsing".format(
                    bulk_task["task_label"], bulk_task["task_id"]
                )
            )
        ...
        if "outcar" in bulk_task["calcs_reversed"][0]["output"].keys():
            ...
            self.logger.error(
                "BULKTYPEcalc: {} (task-id {}) does not "
                "have outcar values for parsing".format(
                    bulk_task["task_label"], bulk_task["task_id"]
                )
            )
        return parameters
    ...
    def get_bulk_gap_data(self, bulk_task, parameters):
        ...
        try:
            ...
            raise ValueError(
                "Error with querying MPRester for {}".format(
        ...
        if (
            mpid
            and not parameters["task_level_metadata"]["incar_calctype_summary"][
                "LHFCALC"
            ]
        ):
        ...
        parameters.update({"mpid": mpid, "cbm": cbm, "vbm": vbm, "gap": gap})
    ...
    def load_defect_and_structure_data(self, defect_task, parameters):
        ...
        defect = MontyDecoder().process_decoded(
            {k: v for k, v in defect.items() if k in needed_keys}
        )

        scaling_matrix = MontyDecoder().process_decoded(
            defect_task["transformations"]["history"][0]["scaling_matrix"]
        )
        ...
        if not ids_sm.fit(ids, initial_defect_structure):
            self.logger.error(
                "Could not match initial-to-final structure. Will not load initial structure."
            )
    ...
    def load_defect_chg_correction_metadata(self, defect, defect_task, parameters):
        ...
        if "locpot" in defect_task["calcs_reversed"][0]["output"].keys():
            ...
            self.logger.error(
                "DEFECTTYPEcalc: {} (task-id {}) does not have locpot values for "
                "parsing Freysoldt correction".format(
                    defect_task["task_label"], defect_task["task_id"]
                )
            )
        ...
        if (
            "outcar" in defect_task["calcs_reversed"][0]["output"].keys()
            and parameters["initial_defect_structure"]
        ):
            ...
            if len(set(np.array(site_matching_indices)[:, 0])) != len(
                ...
                self.logger.error(
                    "Error occured in site_matching routine. Double counting of site matching "
                    "occured:{}\nAbandoning Kumagai parsing.".format(
                        site_matching_indices
                    )
                )
                found_defect = False
            ...
            if found_defect:
                ...
                self.logger.error(
                    "Error in mapping procedure for bulk to initial defect structure."
                )

        else:
            self.logger.error(
                "DEFECTTYPEcalc: {} (task-id {}) does not have outcar values for "
                "parsing Kumagai".format(
                    defect_task["task_label"], defect_task["task_id"]
                )
            )
...
class DefectThermoBuilder(Builder):
    ...
    def get_items(self):
        ...
        if not self.update_all:
            ...
            prev_dpd = list(
                self.defectthermo.query(
                    properties=["metadata.all_entry_ids_considered"]
                )
            )
            ...
            if "entry_id" not in q.keys():
                ...
                q["entry_id"]["$nin"] = []
            for dpd in prev_dpd:
                q["entry_id"]["$nin"].extend(
                    list(dpd["metadata"]["all_entry_ids_considered"])
                )

            self.logger.debug(
                "query after removing previously considered entry_ids is: {}".format(q)
            )
        ...
        entry_keys_needed_for_thermo = [
            "defect",
            "parameters.task_level_metadata",
            ...
            "parameters.vbm",
            "parameters.gap",
            "parameters.hybrid_cbm",
            "parameters.hybrid_vbm",
            "parameters.hybrid_gap",
            "parameters.potalign",
            "parameters.freysoldt_meta",
            "parameters.kumagai_meta",
            "parameters.is_compatible",
            "parameters.delocalization_meta",
            "parameters.phasediagram_meta",
        ]
        defect_entries = list(
            self.defects.query(criteria=q, properties=entry_keys_needed_for_thermo)
        )
        thermo_entries = list(
            self.defectthermo.query(
                properties=[
        ...
        self.logger.info(
            "Found {} new defect entries to consider".format(len(defect_entries))
        )
        ...
        for entry_dict in defect_entries:
            ...
            if "potcar_summary" in entry_dict_md:
                ...
                pspec, plab = set(), set()
                for symbol, full_potcar_label in zip(
                    run_metadata["pot_labels"], run_metadata["pot_spec"]
                ):
                ...
                run_metadata["pot_spec"] = "-".join(
                    sorted(set(pspec))
                )  # had to switch to this approach for proper dict comparison later on
            ...
            if (
                not matched
            ):  # have not logged yet, see if previous thermo phase diagram was created for this system
                for t_ent in thermo_entries:
                    if (
                        t_ent["bulk_chemsys"] == bulk_chemsys
                        and t_ent["run_metadata"] == run_metadata
                    ):
                        if sm.fit(
                            base_bulk_struct,
                            Structure.from_dict(t_ent["bulk_prim_struct"]),
                        ):
                            if not self.update_all:
                                # if not wanting to update everything, then get previous entries from thermo database
                                old_entry_set = list(
                                    self.defectthermo.query(
                                        {"entry_id": t_ent["entry_id"]},
                                        properties=["entries"],
                                    )
                                )[0]["entries"]
                                old_entries_list = [
                                    entry.as_dict() if type(entry) != dict else entry
                                    for entry in old_entry_set
                                ]
                            else:
                                old_entries_list = []

                            old_entries_list.append(entry_dict.copy())
                            grpd_entry_list.append(
                                [
                                    bulk_chemsys,
                                    run_metadata.copy(),
                                    base_bulk_struct.copy(),
                                    old_entries_list,
                                    t_ent["entry_id"],
                                ]
                            )
    ...
    def process_item(self, items):
        ...
        pdc = PointDefectComparator(
            check_charge=True, check_primitive_cell=True, check_lattice_scale=False
        )
        for entry_dict in entrylist:
            entry = DefectEntry.from_dict(
                {k: v for k, v in entry_dict.items() if k in needed_entry_keys}
            )
            matched = False
            ...
            if matched:
                ...
                nxt_ind = (
                    max(distinct_entries.keys()) + 1
                    if len(distinct_entries.keys())
                    else 0
                )
        ...
        for ident_entries in distinct_entries_full:
            ...
            for ent_ind, ent in enumerate(ident_entries):
                try:
                    lu = ent["parameters"]["task_level_metadata"][
                        "defect_task_last_updated"
                    ]
                except:
                    lu = ent["parameters"]["last_updated"]
                if isinstance(
                    lu, dict
                ):  # deal with case when datetime objects were not properly loaded
            ...
            try:
                ...
                for_print_lu_list = [
                    [lu, ident_entries[ent_ind]["entry_id"]] for lu, ent_ind in lu_list
                ]
                raise ValueError(
                    "Error with last_updated sorting of list:\n{}".format(
                        for_print_lu_list
                    )
                )
            recent_entry_dict = ident_entries[lu_list[0][1]]

            new_entry = DefectEntry.from_dict(
                {k: v for k, v in recent_entry_dict.items() if k in needed_entry_keys}
            )
        ...
        for ent in entries:
            if vbm != ent.parameters["vbm"]:
                self.logger.error(
                    "Logging vbm = {} (retrieved from {}, task-id {}) but {} "
                    "(task-id {}) has vbm = {}. Be careful with this defectphasediagram "
                    "if these are very different".format(
                        vbm,
                        entries[0].name,
                        entries[0].entry_id,
                        ent.name,
            ...
            if band_gap != ent.parameters["gap"]:
                self.logger.error(
                    "Logging gap = {} (retrieved from {}, task-id {}) but {} "
        ...
        defect_phase_diagram_as_dict.update(
            {
                "bulk_chemsys": bulk_chemsys,
                "bulk_prim_struct": bulk_prim_struct.as_dict(),
