...
from emmet.vasp.elastic import (
    ElasticAnalysisBuilder,
    ElasticAggregateBuilder,
    group_deformations_by_optimization_task,
    group_by_parent_lattice,
    get_distinct_rotations,
    process_elastic_calcs,
    generate_formula_dict,
...
test_tasks = os.path.join(
    module_dir, "..", "..", "..", "test_files", "vasp", "elastic_tasks.json"
)
...
class ElasticAnalysisBuilderTest(unittest.TestCase):
    ...
    def test_get_distinct_rotations(self):
        ...
        for op in ops:
            self.assertTrue(any([np.allclose(op.rotation_matrix, r) for r in rots]))
    ...
    def test_process_elastic_calcs(self):
        ...
        opt_task = {
            "output": {"structure": test_struct.as_dict()},
        ...
        for n, (struct, defo) in enumerate(zip(dss, dss.deformations)):
            ...
            defo_task.update(
                {"transmuter": {"transformation_params": [{"deformation": defo}]}}
            )
    ...
    def test_process_elastic_calcs_toec(self):
        ...
        for calc in derived:
            self.assertTrue(np.allclose(calc["strain"], calc["cauchy_stress"] / -0.5))
...
class ElasticAggregateBuilderTest(unittest.TestCase):
    def setUp(self):
        ...
        self.test_elasticity = MongoStore(
            "test_emmet", "elasticity", key="optimization_task_id"
        )
        self.test_elasticity.connect()
    ...
    def test_update_targets(self):
        processed = [
            self.builder.process_item(item) for item in self.builder.get_items()
        ]
