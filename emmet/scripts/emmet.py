...
def aggregate_by_formula(coll, q, key=None):
    ...
    if key is None:
        for k in aggregation_keys:
            ...
            raise ValueError(
                "could not find aggregation keys",
                aggregation_keys,
                "in",
                coll.full_name,
            )
    return coll.aggregate(
        [
            {"$match": query},
            {"$sort": OrderedDict([("nelements", 1), ("nsites", 1)])},
            {
                "$group": {
                    "_id": "${}".format(key),
                    "structures": {
                        "$push": dict(
                            (k.split(".")[-1], "${}".format(k)) for k in structure_keys
                        )
                    },
                }
            },
            ...
            for f in files:
                if f.endswith(".tar.gz"):
                    ...
                    for vaspdir in get_vasp_dirs(
                        path.replace(".tar.gz", ""), base_path, max_dirs, insert
                    ):
                        yield vaspdir
                        counter += 1
                        if counter >= max_dirs:
...
def ensure_meta(snl_coll):
    ...
    if docs.count() > 0:
        ...
        for idx, doc in enumerate(docs):
            ...
            snl_coll.update(
                {"snl_id": doc["snl_id"]}, {"$set": get_meta_from_structure(struct)}
            )
...
@click.option(
    "--insert/--no-insert", default=False, help="actually execute task addition"
)
...
@click.option(
    "--src", help="mongogrant string for source task db (overwrite default lpad)"
)
@click.option(
    "--force/--no-force", default=False, help="force overwrite existing target task"
)
def copy(target_spec, tag, insert, copy_snls, sbxn, src, force):
    ...
    ensure_indexes(
        ["task_id", "tags", "dir_name", "retired_task_id"],
    ...
    if tag is None:
        tags = [
            t
            for t in source.collection.find(task_base_query).distinct("tags")
            if t is not None and t not in year_tags
        ]
...
@click.option(
    "--add_tasks_db",
    type=click.Path(exists=True),
    help="config file for additional tasks collection to scan",
)
def find(email, add_snlcolls, add_tasks_db):
    ...
    if add_snlcolls is not None:
        for snl_db_config in yaml.load_all(open(add_snlcolls, "r")):
            snl_db_conn = MongoClient(
                snl_db_config["host"], snl_db_config["port"], j=False, connect=False
            )
...
@click.option(
    "--insert/--no-insert", default=False, help="actually execute workflow addition"
)
def bandstructure(target_db_file, insert):
    ...
    for mat_id in mat_ids:
        structure = Structure.from_dict(
            materials.find_one({"task_id": mat_id}, {"structure": 1})["structure"]
        )
        dir_name = target.collection.find_one({"task_id": mat_id}, {"dir_name": 1})[
            "dir_name"
        ]
        ...
        wf = wf_bandstructure(
            structure, c={"ADD_MODIFY_INCAR": True}
        )  # TODO non-SO bandstructure workflow -> Alex
        wf = add_trackers(wf)
        wf = add_tags(wf, tags)
...
@click.option(
    ...
    "-a",
    multiple=True,
    help="mongogrant string for additional SNL collection to scan",
)
@click.option(
    "--add_taskdbs",
    "-t",
    multiple=True,
    help="mongogrant string for additional tasks collection to scan",
)
...
@click.option(
    ...
    default=False,
    help="clear MongoDB logs collection for specific tag",
)
@click.option(
    "--max-structures", "-m", default=1000, help="set max structures for tags to scan"
)
@click.option(
    "--skip-all-scanned/--no-skip-all-scanned",
    default=False,
    help="skip all already scanned structures incl. WFs2Add/Errors",
)
@click.option(
    "--force-new/--no-force-new", default=False, help="force generation of new workflow"
)
def wflows(
    add_snlcolls,
    add_taskdbs,
    ...
    mongo_handler = MongoHandler(
        host=lpad.host,
        port=lpad.port,
        database_name=lpad.name,
        collection="add_wflows_logs",
        username=lpad.username,
        password=lpad.password,
        authentication_db=lpad.name,
    ...
    ensure_indexes(
        ["level", "message", "snl_id", "formula", "tags"], [mongo_handler.collection]
    )
    ...
    if tag is None:
        ...
        for snl_coll in snl_collections:
            ...
            for t in set(projects_remarks):
                q = {
                    "$and": [
                        {"$or": [{"about.remarks": t}, {"about.projects": t}]},
                        exclude,
                    ]
                }
                q.update(base_query)
        ...
        query = {
            "$and": [
                {"$or": [{"about.remarks": tag}, {"about.projects": tag}]},
                exclude,
            ]
        }
        query.update(base_query)
        ...
        if total:
            ...
            tags[tag] = [
                total,
                to_scan,
                [snl_coll for idx, snl_coll in enumerate(snl_collections) if cnts[idx]],
            ]
    ...
    print(
        "\n".join(
            ["{} ({}) --> {} TO SCAN".format(k, v[0], v[1]) for k, v in tags.items()]
        )
    )
    ...
    def load_canonical_task_structures(formula, full_name):
        ...
        if formula not in canonical_task_structures[full_name]:
            ...
            tasks = tasks_collections[full_name].find(
                task_query, {"input.structure": 1, "task_id": 1, "orig_inputs": 1}
            )
            if tasks.count() > 0:
                task_structures = {}
                for task in tasks:
                    ...
                    if task_label == "Structure Optimization":
                        ...
                        try:
                            ...
                            s.to(
                                fmt="json",
                                filename="sgnum_{}.json".format(task["task_id"]),
                            )
                ...
                if task_structures:
                    for sg, slist in task_structures.items():
                        canonical_task_structures[full_name][formula][sg] = [
    ...
    def find_matching_canonical_task_structures(formula, struct, full_name):
        matched_task_ids = []
        if (
            sgnum in canonical_task_structures[full_name][formula]
            and canonical_task_structures[full_name][formula][sgnum]
        ):
    ...
    for tag, value in tags.items():
        ...
        for coll in value[-1]:
            ...
            structure_groups = aggregate_by_formula(
                coll, {"$or": [{"about.remarks": tag}, {"about.projects": tag}]}
            )
            ...
            try:
                for idx_group, group in enumerate(structure_groups):
                    ...
                    for dct in group["structures"]:
                        ...
                        log_entries = list(
                            mongo_handler.collection.find(q)
                        )  # log entries for already inserted workflows
                        if log_entries:
                            if force_new:
                                q[
                                    "tags"
                                ] = tag  # try avoid duplicate wf insertion for same tag even if forced
                                log_entry = mongo_handler.collection.find_one(
                                    q,
                                    {
                                        "_id": 0,
                                        "message": 1,
                                ...
                                if log_entry:
                                    print(
                                        "WF already inserted for SNL {} with tag {}".format(
                                            dct["snl_id"], tag
                                        )
                                    )
                                    print(log_entry)
                                ...
                                lpad.db.add_wflows_logs.update(
                                    q, {"$addToSet": {"tags": tag}}
                                )
                                ...
                                for s in g[1:]:
                                    logger.warning(
                                        ...
                                        extra={
                                            ...
                                            "snl_id": s.snl_id,
                                            "tags": [tag],
                                            "canonical_snl_id": g[0].snl_id,
                                        },
                                    )

                    if not canonical_structures[formula]:
                        continue
                    canonical_structures_list = [
                        x
                        for sublist in canonical_structures[formula].values()
                        for x in sublist
                    ]

                    if not force_new and formula not in canonical_workflow_structures:
                        (
                            canonical_workflow_structures[formula],
                            grouped_workflow_structures[formula],
                        ) = ({}, {})
                        workflows = lpad.workflows.find(
                            {"metadata.formula_pretty": formula},
                            {"metadata.structure": 1, "nodes": 1, "parent_links": 1},
                        )
                            ...
                            for wf in workflows:
                                ...
                                if sgnum in canonical_structures[formula]:
                                    ...
                                    s.fw_id = [
                                        n
                                        for n in wf["nodes"]
                                        if str(n) not in wf["parent_links"]
                                    ][
                                        0
                                    ]  # first node = SO firework
                                    workflow_structures[sgnum].append(s)
                            if workflow_structures:
                                for sgnum, slist in workflow_structures.items():
                                    grouped_workflow_structures[formula][sgnum] = [
                                        g for g in group_structures(slist)
                                    ]
                                    canonical_workflow_structures[formula][sgnum] = [
                                        g[0]
                                        for g in grouped_workflow_structures[formula][
                                            sgnum
                                        ]
                                    ]
                                # print(sum([len(x) for x in canonical_workflow_structures[formula].values()]), 'canonical workflow structure(s) for', formula)

                    for idx_canonical, (sgnum, slist) in enumerate(
                        canonical_structures[formula].items()
                    ):
                        ...
                        for struc in slist:
                            ...
                            if (
                                not force_new
                                and sgnum in canonical_workflow_structures[formula]
                                and canonical_workflow_structures[formula][sgnum]
                            ):
                                for sidx, s in enumerate(
                                    canonical_workflow_structures[formula][sgnum]
                                ):
                                    if structures_match(struct, s):
                                        msg = "Structure for SNL {} already added in WF {}".format(
                                            struct.snl_id, s.fw_id
                                        )
                                        print(msg)
                                        if struct.task_id is not None:
                                            task_query = {"task_id": struct.task_id}
                                            task_query.update(task_base_query)
                                            for full_name in reversed(
                                                tasks_collections
                                            ):
                                                task = tasks_collections[
                                                    full_name
                                                ].find_one(
                                                ...
                                                if task:
                                                    break
                                            if task:
                                                s_task = Structure.from_dict(
                                                    task["input"]["structure"]
                                                )
                                                s_task.remove_oxidation_states()
                                                if not structures_match(struct, s_task):
                                                    msg = "  --> ERROR: Structure for SNL {} does not match {}".format(
                                                        struct.snl_id, struct.task_id
                                                    )
                                                    msg += "  --> CLEANUP: remove task_id from SNL"
                                                    print(msg)
                                                    coll.update(
                                                        {"snl_id": struct.snl_id},
                                                        {
                                                            "$unset": {
                                                                "about._materialsproject.task_id": 1
                                                            }
                                                        },
                                                    )
                                                    logger.warning(
                                                    ...
                                                    msg = "  --> OK: workflow resulted in matching task {}".format(
                                                        struct.task_id
                                                    )
                                                    print(msg)
                                                    logger.warning(
                                                ...
                                                fw_ids = [
                                                    x.fw_id
                                                    for x in grouped_workflow_structures[
                                                        formula
                                                    ][
                                                        sgnum
                                                    ][
                                                ...
                                                for fw in fws:
                                                    if (
                                                        ...
                                                        msg = "  --> OK: workflow {} will result in intended task-id {}".format(
                                                            fw["fw_id"], struct.task_id
                                                        )
                                                        print(msg)
                                                        logger.warning(
                                                            msg,
                                                ...
                                                if not fw_found:
                                                    print(
                                                        "  --> no WF with enforced task-id",
                                                        struct.task_id,
                                                    )
                                                    fw = lpad.fireworks.find_one(
                                                        {"fw_id": s.fw_id}, {"state": 1}
                                                    )
                                                    ...
                                                    if fw["state"] == "COMPLETED":
                                                        ...
                                                        full_name = list(
                                                            tasks_collections.keys()
                                                        )[0]
                                                        load_canonical_task_structures(
                                                            formula, full_name
                                                        )
                                                        matched_task_ids = find_matching_canonical_task_structures(
                                                            formula, struct, full_name
                                                        )
                                                        if len(matched_task_ids) == 1:
                                                            tasks_collections[
                                                                ...
                                                                },
                                                                {
                                                                    "$set": {
                                                                        "task_id": struct.task_id,
                                                                        "retired_task_id": matched_task_ids[
                                                                            0
                                                                        ],
                                                                        "last_updated": datetime.utcnow(),
                                                                    },
                                                            ...
                                                            msg = "  --> ERROR: multiple tasks {} for completed WF {}".format(
                                                                matched_task_ids,
                                                                s.fw_id,
                                                            )
                                                            print(msg)
                                                            logger.error(
                                                                ...
                                                                extra={
                                                                    ...
                                                                    "snl_id": struct.snl_id,
                                                                    "tags": [tag],
                                                                    "error": "Multiple tasks for Completed WF",
                                                                },
                                                            )
                                                        else:
                                                            msg = "  --> ERROR: task for completed WF {} does not exist!".format(
                                                                s.fw_id
                                                            )
                                                            msg += " --> CLEANUP: delete {} WF and re-add/run to enforce task-id {}".format(
                                                                fw["state"],
                                                                struct.task_id,
                                                            )
                                                            print(msg)
                                                        ...
                                                        print(
                                                            "  --> CLEANUP: delete {} WF and re-add to include task_id as additional_field".format(
                                                                fw["state"]
                                                            )
                                                        )
                                                        lpad.delete_wf(s.fw_id)
                            ...
                            if not force_new:
                                ...
                                for full_name in reversed(tasks_collections):
                                    ...
                                    if (
                                        struct.task_id is not None
                                        and matched_task_ids[full_name]
                                        and struct.task_id
                                        not in matched_task_ids[full_name]
                                    ):
                                        msg = "  --> WARNING: task {} not in {}".format(
                                            struct.task_id, matched_task_ids[full_name]
                                        )
                                        print(msg)
                                ...
                                if any(matched_task_ids.values()):
                                    logger.warning(
                                        ...
                                        extra={
                                            ...
                                            "task_id(s)": dict(
                                                (k.replace(".", "#"), v)
                                                for k, v in matched_task_ids.items()
                                            ),
                                        },
                                    )
                                    continue

                            no_potcars = set(NO_POTCARS) & set(
                                struct.composition.elements
                            )
                            if len(no_potcars) > 0:
                                msg = "Structure for SNL {} --> NO POTCARS: {}".format(
                                    struct.snl_id, no_potcars
                                )
                                print(msg)
                                logger.warning(
                                    msg,
                            ...
                            try:
                                wf = wf_structure_optimization(
                                    struct, c={"ADD_MODIFY_INCAR": True}
                                )
                                wf = add_trackers(wf)
                                wf = add_tags(wf, [tag, year_tags[-1]])
                                if struct.task_id is not None:
                                    wf = add_additional_fields_to_taskdocs(
                                        wf, update_dict={"task_id": struct.task_id}
                                    )
                            except Exception as ex:
                                msg = "Structure for SNL {} --> SKIP: Could not make workflow --> {}".format(
                                    struct.snl_id, str(ex)
                                )
                                print(msg)
                                logger.error(
                                    msg,
                                    extra={
                                        ...
                                        "snl_id": struct.snl_id,
                                        "tags": [tag],
                                        "error": "could not make workflow",
                                    },
                                )
                                continue

                            msg = "Structure for SNL {} --> ADD WORKFLOW".format(
                                struct.snl_id
                            )
                            if struct.task_id is not None:
                                msg += " --> enforcing task-id {}".format(
                                    struct.task_id
                                )
                            print(msg)
                ...
                sites_elements = set(
                    [
                        (
                            len(set([e.symbol for e in x.composition.elements])),
                            x.num_sites,
                        )
                ...
                print(
                    len(canonical_structures_list),
                    "canonical structure(s) for",
                    formula,
                    sites_elements,
                )
                if tag is not None:
...
@click.option("--tag", default=None, help="only include structures with specific tag")
@click.option(
    "--in-progress/--no-in-progress", default=False, help="show in-progress only"
)
...
def report(tag, in_progress, to_csv):
    ...
    if tag is None:
        tags = [
            t
            for t in lpad.workflows.distinct("metadata.tags")
            if t is not None and t not in year_tags
        ]
        tags += [
            t
            for t in lpad.db.add_wflows_logs.distinct("tags")
    ...
    table = PrettyTable()
    table.field_names = (
        ["Tag", "SNLs", "WFs2Add", "WFs"] + states + ["% FIZZLED", "Progress"]
    )
    sums = ["total"] + [0] * (len(table.field_names) - 1)
    ...
    for t in tags:
        ...
        wflows_to_add = lpad.db.add_wflows_logs.count(
            {"tags": t, "level": "ERROR", "error": {"$exists": 0}}
        )
        ...
        if wflows_to_add or counter["COMPLETED"] + counter["FIZZLED"] != total:
            ...
            progress = (
                (counter["COMPLETED"] + counter["FIZZLED"]) / total * 100.0
                if total
                else 0.0
            )
            ...
            continue
        entry = [tc, nr_snls, wflows_to_add, total] + [
            counter[state] for state in states
        ]
        fizzled = counter["FIZZLED"] / total if total else 0.0
        if progress != "-" and bool(counter["COMPLETED"] + counter["FIZZLED"]):
            fizzled = counter["FIZZLED"] / (counter["COMPLETED"] + counter["FIZZLED"])
        sfmt = (
            "\033[1;31m{:.0f}%\033[0m" if (not to_csv and fizzled > 0.2) else "{:.0f}%"
        )
        percent_fizzled = sfmt.format(fizzled * 100.0)
...
@click.option(
    ...
    "-a",
    type=click.Path(exists=True),
    help="YAML config file with multiple documents defining additional SNLs collections to check against",
)
@click.option(
    "--insert/--no-insert", default=False, help="actually execute SNL insertion"
)
...
def add_snls(tag, input_structures, add_snlcolls, insert):
    ...
    for idx, istruct in enumerate(input_structures):
        ...
        if index is None:
            index = (
                max(
                    [
                        int(snl_id[len(prefix) + 1 :])
                        for snl_id in snl_collections[0].distinct("snl_id")
                    ]
                )
                + 1
            )
        ...
        if isinstance(istruct, TransformedStructure):
            ...
            snl = StructureNL(
                istruct, meta["authors"], references=references, projects=[tag]
            )
        snl_dct = snl.as_dict()
                ...
                if ".json" not in launcher["name"]:
                    if ".tar.gz" in launcher["name"]:
                        ...
                        if sync_nomad:
                            ...
                            nomad_query = 'alltarget repository_filepaths.split="{}"'.format(
                            ...
                            if "meta" in resp:
                                ...
                                if (
                                    resp["meta"]["total_hits"] < 1
                                ):  # calculation not found in NoMaD repo
                                    print("Retrieve", path, "...")
                                    ...
                                    for d in resp["data"]:
                                        print(
                                            "\t",
                                            d["attributes"]["repository_archive_gid"],
                                        )
                                    sys.exit(0)
                            else:
