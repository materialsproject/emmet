...
onhull_prod = db_prod.materials.distinct(
    "task_id", {"e_above_hull": 0, "deprecated": {"$ne": True}}
)
onhull_stag = db_stag.materials.distinct(
    "task_id", {"e_above_hull": 0, "deprecated": False}
)
...
criteria = {
    "task_id": {"$in": list(onhull_prod_but_not_stag)},
    "error": {"$exists": False},
}
...
for i, doc in tqdm(
    enumerate(cursor), total=db_stag.materials.count_documents(criteria)
):
    broken = doc["task_id"]
    chemsys = doc["chemsys"]
    chemsys_space = get_chemsys_space(chemsys)
    to_be_on_hull = list(
        db_stag.materials.find(
            {"chemsys": {"$in": chemsys_space}, "e_above_hull": 0, "deprecated": False},
            ["task_id", "final_energy_per_atom"],
        )
    )
    existing_mats = list(
        db_prod.materials.find(
            {"task_id": {"$in": [d["task_id"] for d in to_be_on_hull]}},
            ["task_id", "final_energy_per_atom"],
        )
    )
    new_materials = {d["task_id"] for d in to_be_on_hull} - {
        d["task_id"] for d in existing_mats
    }
    ...
    if not new_materials:
        ...
        for mat in existing_mats:
            new_final_e_per_atom = next(
                m["final_energy_per_atom"]
                for m in to_be_on_hull
                if m["task_id"] == mat["task_id"]
            )
            if (
                not np.allclose(
                    [mat["final_energy_per_atom"]], [new_final_e_per_atom], atol=0.0005
                )  # 0.5 meV
                ...
                lower_e.append(
                    dict(
                        mat=mat["task_id"],
                        e_change=round(
                            mat["final_energy_per_atom"] - new_final_e_per_atom, 4
                        ),
                    )
                )
        higher_e = []
        if len(lower_e) == 0:
            ...
            broken_old_final_epa = db_prod.materials.find_one(
                {"task_id": broken, "deprecated": {"$ne": True}},
                ["final_energy_per_atom"],
            )["final_energy_per_atom"]
            try:
                broken_new_final_epa = db_stag.materials.find_one(
                    {"task_id": broken, "deprecated": False}, ["final_energy_per_atom"]
                )["final_energy_per_atom"]
            except TypeError:  # Newly deprecated
                continue
            higher_e = dict(
                old_mat=broken,
                e_change=round(broken_new_final_epa - broken_old_final_epa, 4),
            )
            breaks_via_old_mats_higher_e.append(
                dict(
                    mat=broken,
                    e_change_higher=round(
                        broken_new_final_epa - broken_old_final_epa, 4
                    ),
                    chemsys=chemsys,
            ...
            breaks_via_old_mats_lower_e.append(
                dict(
        ...
        try:
            new_hull_e = db_stag.materials.find_one(
                {"task_id": broken, "deprecated": False}, ["e_above_hull"]
            )["e_above_hull"]
        except TypeError:  # Newly deprecated
            continue
        breaks_via_new_mats.append(
            dict(
                mat=broken,
                chemsys=chemsys,
...
def as_html(b):
    ...
    if "new_hull_e" in b:
        ...
        old_mats = [
            "{} (e_change={})".format(mat_link(changed["mat"]), changed["e_change"])
...
print(
    f"{len(breaks_via_old_mats_lower_e)} hull breaks due to now-lower-energy existing materials\n"
)
if breaks_via_old_mats_lower_e:
    print(f"In decreasing order of energy change:")
    breaks_via_old_mats_lower_e = sorted(
        breaks_via_old_mats_lower_e,
        key=lambda b: b["lower_e"][0]["e_change"],
        reverse=True,
    )
...
print(
    f"{len(breaks_via_old_mats_higher_e)} hull breaks due to now-higher-energy existing materials\n"
)
