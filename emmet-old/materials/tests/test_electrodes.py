...
test_mats = os.path.join(
    module_dir, "..", "..", "..", "test_files", "insert_electrode.json"
)
...
class TestElectroInsert(unittest.TestCase):
    def setUp(self):
        ...
        self.builder = ElectrodesBuilder(
            materials=self.materials, electro=self.insert_electrode, working_ion="Li"
        )
    ...
    def test_mat_doc2comp_entry(self):
        ...
        self.assertEqual(
            len(self.builder._mat_doc2comp_entry(mat_docs)), 168
        )  # documents in the json file
    ...
    def test_get_hashed_entries_from_chemsys(self):
        ...
        for ii in hashed_entries:
            all_comps = [
                get_no_li_comp(ient.composition) for ient in ii["elec_entries"]
            ]
    ...
    def test_group_entries(self):
        for ii in self.builder.get_hashed_entries_from_chemsys("Bi-Li-O"):
            ...
            for ig in groups:
                ...
                self.assertEqual(
                    max(generic_groupby(structs_in_group, self.builder.sm.fit)), 0
                )
    ...
    def test_get_items(self):
        for item in self.builder.get_items():
            ...
            pd_chemsys = [
                "-".join(sorted(ient.composition.as_dict().keys()))
                for ient in item["pd_entries"]
            ]
    ...
    def test_get_competing_conversion_electrode(self):
        for item in self.builder.get_items():
            ents = sorted(
                item["elec_entries"],
                key=lambda x: x.composition.get_atomic_fraction(
                    self.builder.working_ion
                ),
            )
    ...
    def test_update_targets(self):
        items = [
            [{"battid": 1, "_sbxn": ["core"]}] * 3,
            [{"battid": 2, "_sbxn": ["core"]}] * 4,
            [{"battid": 3, "_sbxn": ["core"]}] * 4,
        ]
        tbuilder = ElectrodesBuilder(
            materials=self.materials, electro=self.insert_electrode, working_ion="Li"
        )
