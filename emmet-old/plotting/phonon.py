...
class PhononDispersionPlotter(Builder):
    def __init__(
        self, pmg_docs, web_docs, images, add_filter=None, ignore_lu=False, **kwargs
    ):
        """
        ...
        super().__init__(sources=[pmg_docs], targets=[web_docs, images], **kwargs)
    ...
    def get_items(self):
        ...
        if cursor.count() == 0:
            ...
            self.logger.debug(
                "{} updated pmg_docs that do not match"
    ...
    def update_targets(self, items):
        ...
        web_docs = [
            {"mp-id": item["mp_id"], "ph_bs": item["web_doc"]} for item in items
        ]
        self.web_docs.update(web_docs)
        images = [{"mp-id": item["mp_id"], "plot": item["image"]} for item in items]
    ...
    def validate_targets(self, n=0):
        self.logger.info(
            "Validating {}".format("all" if not n else "with sample size {}".format(n))
        )
