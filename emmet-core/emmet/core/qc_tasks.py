...
from emmet.core.qchem.calculation import(
    Calculation
)
...
__author__ = "Evan Spotte-Smith <ewcspottesmith@lbl.gov>, Rishabh D. Guha <rdguha@lbl.gov>"
...
class OutputDoc(BaseModel):
    ...
    @classmethod
    def from_qchem_calc_doc(
        self, cls, calc_doc: Calculation, **kwargs
    ) -> "OutputDoc":
...
def _parse_orig_inputs(
    ...
    for filename in dir_name.glob("*.orig*"):
        orig_inputs = QCInput.from_file(
            os.path.join(dir_name, filename.pop("orig"))
        )
        return orig_inputs
