...
def phase_diagram_serializer(phase_diagram, nxt, info) -> dict[str, Any]:
    ...
    # legacy deserialization behavior
    legacy_pd_keys = ["all_entries", "qhull_entries", "simplexes"]
    if all(default_serialized_object["computed_data"].get(k) for k in legacy_pd_keys):
        for key in legacy_pd_keys:
            default_serialized_object["computed_data"][key] = [
                entry.as_dict()
                for entry in default_serialized_object["computed_data"][key]
            ]
