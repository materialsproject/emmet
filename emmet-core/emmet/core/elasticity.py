...
class ElasticityData(BaseModel):
    ...
    elastic_tensor: List[List[float]] = Field(
        None, description="Stiffness tensor in GPa.",
    )

    compliance_tensor: List[List[float]] = Field(
        None, description="Compliance tensor in 10^(-12)/Pa.",
    )
...
class ElasticityDoc(BaseModel):
    ...
    pretty_formula: str = Field(
        None, description="Cleaned representation of the material formula",
    )
    ...
    elasticity: ElasticityData = Field(
        None, description="Elasticity data for the material",
    )
