...
class InsertedIonCoords(EmmetBaseModel):
    """
    sub model that deals with typing & parsing bugs
    """

    uc_site_type: int = Field(
        ..., description="index number of the site in uc"
    )

    site: PeriodicSite = Field(
        ..., description="the supercell site"
    )

    extra_site: bool = Field(
        False, description="whether or not this site was generated beyond original sc multiplication"
    )
...
class MigrationGraphDoc(EmmetBaseModel):
    ...
    inserted_ion_coords: List[InsertedIonCoords] = Field(
        None,
    ...
    def generate_sc_fields(
        ...
        combo, coords_list = MigrationGraphDoc.get_hop_sc_combo(
            mg.unique_hops, sc_mat, sm, host_sc, coords_list
        )
    ...
    def get_hop_sc_combo(
        ...
        combo = []
        working_ion = ordered_sc_site_list[0]["site"].species_string
        ...
        for one_hop in unique_hops.values():
            ...
            if not added:
                new_combo, ordered_sc_site_list = MigrationGraphDoc.append_new_site(
                    host_sc, ordered_sc_site_list, one_hop, sc_mat
                )
    ...
    def append_new_site(
        ...
        if sc_iindex is None:
            host_sc_insert.insert(
                0, ordered_sc_site_list[0]["site"].species_string, sc_ipos
            )
        ...
        if sc_eindex is None:
            host_sc_insert.insert(
                0, ordered_sc_site_list[0]["site"].species_string, sc_epos
            )
            ordered_sc_site_list.append({
