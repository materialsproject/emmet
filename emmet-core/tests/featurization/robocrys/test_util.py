...
def test_connected_geometries():
    from emmet.core.featurization.robocrys.util import connected_geometries

    assert "octahedral" in connected_geometries


def test_geometry_to_polyhedra():
    from emmet.core.featurization.robocrys.util import geometry_to_polyhedra

    assert geometry_to_polyhedra["octahedral"] == "octahedra"


def test_polyhedra_plurals():
    from emmet.core.featurization.robocrys.util import polyhedra_plurals

    assert polyhedra_plurals["pentagonal pyramid"] == "pentagonal pyramids"


def test_dimensionality_to_shape():
    from emmet.core.featurization.robocrys.util import dimensionality_to_shape

    assert dimensionality_to_shape[3] == "framework"


def test_superscript_number():
...
def test_load_condense_structure_json(test_condensed_structures):
    ...
    condensed_structure = test_condensed_structures["SnO2"].copy()
