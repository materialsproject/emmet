...
def test_lobster_task_doc(
    lobster_test_dir,
    save_cohp_plots,
    store_lso_dos,
    add_coxxcar_to_task_document,
    tmp_path,
):
    ...
    if add_coxxcar_to_task_document:
        assert {
            *map(
                type,
                (lobster_doc.cohp_data, lobster_doc.cobi_data, lobster_doc.coop_data),
            )
        } == {CompleteCohp}
    else:
        assert {
    ...
    assert lobster_doc.lobsterout.charge_spilling[0] == pytest.approx(
        0.00989999, abs=1e-7
    )
    ...
    assert isinstance(
        lobster_doc.lobsterpy_data.cohp_plot_data.data["Ga1: 4 x As-Ga"], Cohp
    )
    ...
    assert isinstance(
        lobster_doc.lobsterpy_data_cation_anion.cohp_plot_data.data["Ga1: 4 x As-Ga"],
        Cohp,
    )
    ...
    assert {
        *map(
            type,
            (lobster_doc.icohp_list, lobster_doc.icobi_list, lobster_doc.icoop_list),
        )
    } == {Icohplist}
    assert isinstance(lobster_doc.gross_populations, Grosspop)
    assert lobster_doc.chemsys == "As-Ga"

    # delete files generated as part of test exec
    to_delete_ext = ["*.pdf", "*.txt", "*json*"]
    for ext in to_delete_ext:
        for file in (lobster_test_dir / "mp-2534").rglob(ext):
            if file.is_file():
                file.unlink()
