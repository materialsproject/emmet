...
def test_calc_output_from_directory(test_dir):
    ...
    assert np.array_equal(calc_out.steps_reported[2:5], [100, 200, 300])
    assert np.allclose(calc_out.potential_energy[2:5], [-26192.4, -25648.6, -25149.6])
    assert np.allclose(calc_out.kinetic_energy[2:5], [609.4, 1110.4, 1576.4], atol=0.1)
    assert np.allclose(calc_out.total_energy[2:5], [-25583.1, -24538.1, -23573.2])
    assert np.allclose(calc_out.temperature[2:5], [29.6, 54.0, 76.6], atol=0.1)
    assert np.allclose(calc_out.volume[2:5], [21.9, 21.9, 21.9], atol=0.1)
    assert np.allclose(calc_out.density[2:5], [1.0, 0.99, 0.99], atol=0.1)
